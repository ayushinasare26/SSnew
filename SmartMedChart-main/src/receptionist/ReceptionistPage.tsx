...
export default function ReceptionistPage() {
  ...
  const handleSaveAdmission = (patientId: string, admission: PatientAdmission | null | undefined) => {
    setPatients((prev) =>
      prev.map((p) => {
        if (p.id === patientId) {
          const updatedPatient: Patient = {
  ...
  const [initialWardIdForAdmit, setInitialWardIdForAdmit] = useState<string | null>(null);
  const [initialBedForAdmit, setInitialBedForAdmit] = useState<string | null>(null);
  const [registerWithAdmissionDefault, setRegisterWithAdmissionDefault] = useState(false);

  const handleOpenAdmitModal = (patient?: Patient, preselectedWardId?: string, preselectedBed?: string) => {
    if (patient) {
      setPatientForAdmit(patient);
    } else {
      const outpatient = patients.find((p) => !p.admission?.isAdmitted);
      setPatientForAdmit(outpatient || patients[0] || null);
    }
    setInitialWardIdForAdmit(preselectedWardId || null);
    setInitialBedForAdmit(preselectedBed || null);
    setIsAdmitModalOpen(true);
  };

  const handleOpenRegistration = (withAdmission?: boolean | React.MouseEvent) => {
    setRegisterWithAdmissionDefault(typeof withAdmission === 'boolean' ? withAdmission : false);
    ...
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 font-sans antialiased flex flex-row">
        ...
        <main className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 space-y-6">
          ...
          {activeTab === 'overview' && (
            <div className="space-y-6 animate-in fade-in duration-150">
              ...
              <WelcomeHeroBanner
                onOpenQRScanner={() => setIsQRScannerOpen(true)}
                onOpenAddPatient={() => handleOpenRegistration(false)}
                onAdmitInpatient={() => handleOpenAdmitModal()}
            ...
            <div className="space-y-4 animate-in fade-in duration-150">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 rounded-2xl bg-emerald-50/70 border border-emerald-200 shadow-2xs">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <button
                    type="button"
                    id="admitted-view-admit-btn"
                    onClick={() => handleOpenAdmitModal()}
                    className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs shadow-2xs transition-colors cursor-pointer"
                  >
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-white text-emerald-800 border border-emerald-300 font-bold text-xs shadow-2xs hover:bg-emerald-100/50 cursor-pointer"
                  >
          ...
          {activeTab === 'register' && (
            <div className="space-y-4 animate-in fade-in duration-150">
              ...
              <AddPatientCard
                ...
                defaultAdmitToWard={registerWithAdmissionDefault}
          ...
          {activeTab === 'wards' && (
            <div className="space-y-4 animate-in fade-in duration-150">
              <WardsBedMapView
                ...
                onOpenAddPatient={() => handleOpenRegistration(true)}
                onAdmitToBed={(wardId, bedNumber) => handleOpenAdmitModal(undefined, wardId, bedNumber)}
              />
            </div>
          )}
      ...
      <AdmitPatientModal
        ...
        onClose={() => {
          ...
          setInitialWardIdForAdmit(null);
          setInitialBedForAdmit(null);
        }}
        doctors={doctors}
        allPatients={patients}
        initialWardId={initialWardIdForAdmit}
        initialBedNumber={initialBedForAdmit}
        onSaveAdmission={(patientId, admissionData) => {
          handleSaveAdmission(patientId, admissionData);
          if (admissionData?.isAdmitted) {
            handleSelectTab('admitted');
          }
        }}
        onOpenRegistrationWithAdmit={() => handleOpenRegistration(true)}
      />
